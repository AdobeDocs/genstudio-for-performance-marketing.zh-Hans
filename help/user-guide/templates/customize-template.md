...
>[!TIP]
>
>内容占位符&#x200B;**与**&#x200B;与用户以后要填充的内容中的占位符文本不同。 查看有关[在模板](/help/user-guide/templates/best-practices-for-templates.md#using-placeholder-text-in-templates)中使用占位符文本的详细信息。

GenStudio for Performance Marketing可识别模板中的某些内容类型或[元素](use-templates.md#template-elements)，但必须使用[可识别的字段名称](#recognized-field-names)来识别它们。
...
在[!DNL Create]过程中通过富文本编辑增强您的创意内容。 画布根据内容占位符的位置确定富文本功能。 富文本功能仅在将内容占位符用作独立元素或在块级别的HTML标记（如`<p>`、`<div>`或`<span>`）内时才可用。
...
如果在HTML属性中使用内容占位符（如`alt`、`href`或`src`），则该字段不支持富文本编辑。
...
如果字段出现多次，则根据是否在任何实例中将其用作HTML属性来确定富文本功能。 例如，当标题用作标题，用作图像的替换文本时，`alt`标记优先。

富文本编辑&#x200B;**不**&#x200B;可用于`headline`，因为它用作`alt`内容：
...
富文本编辑可能适用于特定渠道内的某些字段，例如社交渠道(Meta、LinkedIn)中的`on_image_text`。
...
请参阅[模板代码编辑器](/help/user-guide/templates/code-editor.md)。
...
如果在创建初始预览后源发生更改，请使用[refresh](/help/user-guide/templates/use-templates.md#refresh-template)函数以外部源内容的最新版本更新模板预览。
