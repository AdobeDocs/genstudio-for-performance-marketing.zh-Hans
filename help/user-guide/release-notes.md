...
对于[!DNL Insights]中的Meta广告营销活动，您现在可以使用与优化营销活动的方式匹配的转化操作来分析每次操作的成本(CPA)，而不是只使用单个默认量度。

从可用的Meta操作类型中进行选择，例如：
...
并在广告、Assets （媒体）和属性[!DNL Insights]表中以一致的方式审核CPA。
...
Innovid现在在[!DNL Insights]中显示为显示渠道，因此您可以在一个位置查看Meta、LinkedIn、TikTok以及其他连接的源旁边的营销活动和广告效果，并且每天都会刷新指标。

使用Innovid评估展示次数、点击量和点进率。

### 通过单个视频资产激活LinkedIn

现在，您可以按照与静态LinkedIn广告相同的端到端激活流程，从GenStudio for Performance Marketing将使用单个视频资源](/help/user-guide/activation/activate-linkedin-ad.md)的LinkedIn广告体验[激活到LinkedIn促销活动管理器。

### 针对付费媒体和电子邮件体验的现成翻译

现在使用[创建主页](/help/user-guide/create/overview.md)上的&#x200B;**[!UICONTROL 翻译和本地化]**&#x200B;卡片，或者在[管理资产和体验](/help/user-guide/content/manage-assets.md)中从已批准的付费媒体体验或电子邮件体验的“操作”菜单中选择&#x200B;**[!UICONTROL 翻译]**&#x200B;以翻译内容

对于付费媒体，请将内容翻译为&#x200B;**40+**&#x200B;种语言。

对于电子邮件，请在单个流程中将内容翻译为&#x200B;**超过10**&#x200B;种支持的语言。

### 模板资产的第三方DAM连接

组织现在可以使用注册的加载项来&#x200B;**连接自己的数字资产管理(DAM)**&#x200B;存储库，以便作者可以在模板参数流中浏览并从该系统选择媒体。

请参阅[查找DAM扩展](/help/extensibility/deploy-app.md#find-dam-extensions)和[开始构建可扩展的应用程序](/help/extensibility/setup.md)，以了解如何构建和分发加载项。

### 用于集成的Experience Selector微前端

**Experience Selector**&#x200B;将GenStudio引入合作伙伴和自定义应用程序中。 嵌入微前端(MFE)，以在合作伙伴或自定义应用程序中提供符合您集成要求的、具有一致UI和登录的浏览、筛选和选择功能。

在[GenStudio Experience Selector MFE](/help/extensibility/experience-selector.md)中采用独立捆绑包、配置选项和嵌入模式。 您的团队会提供选取器体验，而无需从头开始重新构建选择UX。

### Firefly Image Model 4上的自定义模型

在[!DNL Create]中[生成图像](/help/user-guide/create/generate-assets.md)。 **[!UICONTROL 模型]列表现在包含在Adobe Firefly**&#x200B;图像模型4上针对&#x200B;**品牌**&#x200B;和&#x200B;**文本到图像**&#x200B;工作进行培训的自定义模型，这些模型反映的是图像模型4的培训情况，而不只是基于图像模型3的旧自定义模型。

您仍可停留在相同的[!DNL Create]流程中，而较新的Firefly基础支持使用经过图像模型4训练的模型来进行品牌安全生成。 参数和提示的行为与其他模型选取类似 — 没有可直立的并行工作流。

### Horizon画布上的快速模板（Meta、LinkedIn和Display）

**Horizon画布** + Adobe Express模板现在为[!DNL Create]中的&#x200B;**Meta**、**LinkedIn**&#x200B;和&#x200B;**显示**&#x200B;的&#x200B;**正式提供**。 多尺寸布局、移动就绪型Express设计（如果支持）、图层锁定和WYSIWYG编辑可替代这些渠道的旧HTML模板瓶颈，因此团队可以更快地提供品牌创意。

阅读[使用GenStudio for Performance Marketing中的Adobe Express模板](/help/user-guide/templates/express-templates.md)以了解支持的格式、入门和自定义模板以及画布功能。 本主题说明了扩展Express布局时提供的特定于渠道的支持。

### PDF导出以供审阅和存档

[!BADGE Beta]{type=Informative tooltip="此功能当前位于Beta中，因此某些功能可能会受到限制或发生更改。"}

您可以[从[!DNL Create]画布上的导出草稿和[!DNL Content]中的已批准体验中将体验导出到PDF](/help/user-guide/content/manage-assets.md#export-experiences)，从而生成一个不可编辑的文档用于利益相关者审核、批准记录和存档 — 尤其是当团队需要固定快照用于合规性或受管控的审核工作流时。

## 早期发行说明

+++2026.03.15版注释

### Meta、LinkedIn和Display的快速模板

[!DNL Create]现在支持Meta、LinkedIn和Display的Horizon画布上的[快速模板](/help/user-guide/templates/express-templates.md)。 可直接在创建工作流中使用Adobe Express模板（包括入门模板和自定义品牌版面）。

画布支持模板功能，包括：

* 每个模板多种大小
* 在Express中设置的图层锁定和解锁选项
* WYSIWYG编辑模板元素
* 更丰富的文本、媒体交换、徽标替换、视频重新帧等控件

在[使用GenStudio for Performance Marketing中的Adobe Express模板](/help/user-guide/templates/express-templates.md)中探索详细信息。
...
* 通过使用Azure Open AI翻译服务，团队可以将内容同时批量翻译为多种目标语言。
...
* 针对Meta广告促销活动，通过灵活的转化跟踪](/help/user-guide/insights/ads.md)分析[每操作成本(CPA)。
...
* 用户现在可以应用[!DNL Brand]标记，以便按[!DNL Brand]对[!DNL Product]和[!DNL Persona]记录进行分组。
...
现在支持具有相同角色的多个文本字段（例如，“body”、“cta”、“on image text”等） 适用于复杂的客户模板。
...
Adobe GenStudio for Performance Marketing现在提供内置的电子邮件翻译功能，允许营销人员在全球范围内高效地扩展其电子邮件促销活动。 利用此功能，您可以使用Azure Open AI翻译服务获取批准的电子邮件体验并将其翻译成多种目标语言。
...
Meta广告的多纵横比图像激活允许广告商在单个广告创意下上传和激活多个不同纵横比的图像资源。 此功能使一个广告能够为各种Meta投放位置（如信息源、故事和胶卷）提供合适的创意效果。 广告商可以在单个API调用中预览每个图像在投放位置中的呈现方式并将所有版本发布到Meta。
...
* 现在，您可以在平台设置期间通过克隆现有广告集来[创建新的广告集](/help/user-guide/activation/activate-meta-ad.md#create-a-new-ad-set)。 Meta广告集定义了特定广告的时间、渠道详细信息和受众。 Meta营销活动可以包含多个广告集，但一个广告集仅与一个营销活动关联。
...
* 新增模板过滤器选项！ 现在，您可以在[!DNL Create]和&#x200B;_[!UICONTROL 内容]_ > _[!UICONTROL 模板]_&#x200B;中优化&#x200B;_[!UICONTROL 选择模板]_&#x200B;列表。 请参阅[搜索模板](/help/user-guide/templates/use-templates.md#search-templates)。 确保使用元数据正确标记模板，以使这些模板可通过这些过滤器发现。
...
营销人员现在可以将GenStudio for Performance Marketing中的[广告体验](/help/user-guide/activation/overview.md)发布到Meta广告管理器。 [!DNL Activate]支持部署前的详细Meta广告预览。 通过[!DNL Activate]发布的Meta广告一旦上线即自动提取到[!DNL Insights]中，使用户能够跟踪和报告广告效果。
...
作为品牌验证流程的一部分，[内容检查](/help/user-guide/guidelines/brand-validation.md)中引入了[合规性标准](/help/user-guide/guidelines/overview.md)。 这些检查将根据[!DNL Brand]指南、平台指南（如Meta）和ADA标准审查体验中的每个变体。 此过程全面总结了需要修订的准则和标准，以更好地满足合规性要求。
...
* 现在，字符计数将显示在所有生成的和手动的显示广告字段中。 查看[Meta体验](/help/user-guide/create/meta-experiences.md#character-counts)中的&#x200B;_字符数_。<!-- GS-7732 -->
...
* 将展示广告和Meta广告导出为HTML、JPEG或PNG。 请参阅[创建显示广告体验](/help/user-guide/create/create-display-ad.md)和[创建元广告体验](/help/user-guide/create/create-meta-ad.md)。<!-- GS-7093 6655 5152-->
...
* 现在，画布会按正确顺序显示调整过大小的Meta广告和显示广告。 <!-- GS-7375 -->

* 编辑者现在可以单击进入电子邮件和显示广告的字段。<!-- GS-6297 -->

* 电子邮件和Meta广告的编辑片段功能现在会按预期通过一次单击触发。<!-- GS-8081 -->
...
* 现在，用户可以仅通过键盘控制所有Meta和模板交互元素。<!-- GS-4066 -->

* 将图像维度从模板图像字段提取添加到显示广告模板。 智能裁剪请求现在针对图像的实际维度而不是整个模板发送。 <!-- GS-6926 -->

* 已本地化生成的电子邮件和Meta广告中的`Zoom to fit to screen`字符串。<!-- GS-5063 -->
