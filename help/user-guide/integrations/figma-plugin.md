...
[从Figma社区市场](https://www.figma.com/community/plugin/1604251370122180013/firefly-enterprise-and-genstudio)查找并安装插件。
...
>[!NOTE]
>
> 要执行此任务，您需要GenStudio for Performance Marketing的Figma插件和[Photoshop插件](photoshop-plugin.md)。

可以使用Figma插件将一个Figma框架、多个框架或整个文档转换为Photoshop格式，并将其导出以用于[Photoshop插件](photoshop-plugin.md)。 目前，在转换过程中仅支持可视性、字体大小和基本图层属性等主要属性。 目前尚不支持删除线、上标、下标、不透明度百分比、渐变和类似高级属性等功能。
...
1. 执行下列操作之一：
   ...
   * 单击&#x200B;**[!UICONTROL 传输到Photoshop插件]**&#x200B;可缓存转换后的文件，以便在Photoshop插件中立即使用。
     ![“传输到Photoshop插件”按钮](./transfer-to-ps-button.png){width="60%"}
...
1. 单击&#x200B;**[!UICONTROL 提交]**。 该插件读取图中的选定帧，并将其转换为JSON文档（文件数据的一种中间格式）。
   ![需要文件密钥对话框](./file-key-required.png){width="40%"}
1. 在Photoshop中，打开GenStudio插件，然后单击&#x200B;**[!UICONTROL 导入]**&#x200B;选项卡。
1. 执行下列操作之一：

   * 单击&#x200B;**[!UICONTROL 从插件]**&#x200B;以从缓存的文件列表中选择通过&#x200B;**[!UICONTROL 传输到Photoshop插件]**&#x200B;转换的文件，或者
   * 单击&#x200B;**[!UICONTROL 上传JSON]**&#x200B;以浏览并选择要上传的JSON文件。
     ![上传JSON按钮](./upload-json.png){width="40%"}
1. Photoshop中的GenStudio插件会将JSON文档中的信息转换为打开的Photoshop文档。
