...
[!DNL GenStudio for Performance Marketing]的Experience Selector脚本是从`experience.adobe.com`上的Adobe托管URL加载的，而不是从典型实施中的Salesforce静态资源加载的。
...
项目使用Salesforce DX布局；默认包目录为`force-app`。
...
* `force-app/main/default/lwc/sfgsmfe` — LWC包(HTML、JS、CSS、meta)。
* `force-app/main/default/classes/EmailTemplateController.cls` — 用于创建模板的顶点。

存储库可能还包含静态资源(`reactApp`， `sfgsmfe_react`)。 `sfgsmfe.js`中的当前[!DNL GenStudio for Performance Marketing]加载器使用`standalone.js`的Adobe CDN URL；除非您更改实现，否则该加载路径不需要这些静态资源。
...
1. 按照最新的Salesforce闪电文档，添加或调整`https://experience.adobe.com`的&#x200B;**[!UICONTROL 受信任的URL]**（以及Salesforce版本的任何相关设置）。
   ![Salesforce CSP受信任的站点](./sf-trusted-urls.png){width="80%" zoomable="yes"}
...
1. 部署完成，并且没有错误。
1. 用户可以打开包含`sfgsmfe`的Lightning页面并查看Experience Selector UI。
1. 组件未显示加载错误；“网络”选项卡返回`standalone.js`的HTTP 200。
1. **[!UICONTROL 选择GenStudio Experience]**&#x200B;将打开选择器并运行选择回调。
1. 使用该流程时，**[!UICONTROL 创建电子邮件模板]**&#x200B;成功，并且模板出现在&#x200B;**[!UICONTROL 设置]**&#x200B;中已配置的文件夹下。
