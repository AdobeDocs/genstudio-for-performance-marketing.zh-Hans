...
## 特定于项目的约定
- 游标规则：自定义自动化和指导存在于`.cursor/rules/*.mdc`中。 示例： `.cursor/rules/docs-lint.mdc` （lint进程），`.cursor/rules/generate-release-notes.mdc` （编辑`help/user-guide/release-notes.md`时指向发行说明技能）。 对于自动任务，请遵循这些步骤。
- 代理技能：任务工作流在`.cursor/skills/<name>/SKILL.md`下运行。 示例： `.cursor/skills/generate-release-notes/SKILL.md`（完整的GenStudio发行说明过程；在同一文件夹中用于`examples.md`和`reference.md`）。
- 文件名和前端内容：
   - 发行说明：在`help/user-guide/release-notes.md`上保留现有的Experience League frontmatter（请参阅`.cursor/skills/generate-release-notes/reference.md#frontmatter`）；在编辑该文件时，精简规则`.cursor/rules/generate-release-notes.mdc`会自动附加。
...
## 编辑的安全规则（AI应该做什么）
- 切勿将Jira ID、内部链接或仅用于公司的引用添加到公共文档。 请参阅`.cursor/skills/generate-release-notes/SKILL.md`（**禁止的内容**&#x200B;部分）。
